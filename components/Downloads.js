...
async function fetchBuild(buildId, type) {
  ...
  const date = json?.nixname?.match(/(\d{8})/)?.[0]?.replace(/(\d{4})(\d{2})(\d{2})/, "$3.$2.$1");
...
function Download({
  ...
  if (state.error) {
    return (
      <Cards.Card
        icon={<DownloadIcon />}
        title={error}
        href="https://git.oss.uzinfocom.uz/xinux/website/issues"
      />
    );
  }
  ...
  return <Cards.Card icon={<DownloadIcon />} title={state.result.name} href={state.result.link} />;
}
