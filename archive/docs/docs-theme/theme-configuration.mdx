...
You can also use a _function component_ as `head` to dynamically generate the head tags based on the current page’s front matter. For example:
...
You can refer to the [`useConfig`](/docs/docs-theme/api/use-config) API section for more information about the `useConfig` hook and the `frontMatter` object.
...
<div className="flex items-center gap-2 mt-4 h-6">
  <input
    type="range"
    min="0"
    max="360"
...
Check out [Page Configuration](/docs/docs-theme/page-configuration#navbar-items) to learn how to add custom menus or links to the navbar.
...
Show a dismissible banner on the top of the website. It can be used to show a warning or a notice.
...
Together with the [Separators](/docs/docs-theme/page-configuration#separators) item, you can customize how the sidebar content is rendered by using the `sidebar.titleComponent` option:
...
Render top and/or bottom content of main area of the page. It can be used to render a comment section, a newsletter form, or any other type of content.
...
`toc.float` is enabled by default. When enabled, the TOC will be displayed on the right side of the page and it will be sticky when scrolling. If it’s disabled, the TOC will be displayed directly on the page sidebar.
